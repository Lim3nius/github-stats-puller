...
class EventBackfiller:
    ...
    def backfill_from_files(self, dry_run: bool = False) -> Dict[str, int]:
        """
        ...
        for file_path in json_files:
            ...
            try:
                ...
                if event_data_list and not dry_run:
                    # Use ClickHouse-specific insert_event_data method
                    inserted_count = self.clickhouse_service.insert_event_data(event_data_list)
...
def main():
    ...
    parser.add_argument(
        "--events-dir", 
        type=str, 
        help="Directory containing JSON event files (default: downloaded-events)"
    )
    parser.add_argument(
        "--dry-run", 
        action="store_true", 
        help="Process files but don't insert into database"
    )
    parser.add_argument(
        "--dotenv", 
        type=str,
        default="gh.env",
        help="Path to .env file (default: gh.env)"
    )
    
    args = parser.parse_args()
    
    backfiller = EventBackfiller(events_dir=args.events_dir, dotenv_path=args.dotenv)
    stats = backfiller.backfill_from_files(dry_run=args.dry_run)
